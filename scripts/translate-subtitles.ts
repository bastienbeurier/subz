/**
 * translate-subtitles.ts
 *
 * Translates all English video subtitles to French using the Claude API
 * with prompt caching, then inserts the results into Supabase.
 *
 * Usage:
 *   npm install
 *   npx tsx scripts/translate-subtitles.ts
 *
...
Your job is to translate English subtitle texts to French.
Rules:
    ...
    system: [
      {
        type: "text",
        text: SYSTEM_PROMPT,
        cache_control: { type: "ephemeral" }, // cache the system prompt across all requests
      },
    ],
    messages: [
      {
        role: "user",
        content: `Translate these subtitle texts from English to French.\nReturn a JSON array only.\n\n${JSON.stringify(texts, null, 2)}`,
      },
    ],
  });
...
async function main() {
  ...
  for (let i = 0; i < videoIds.length; i++) {
    ...
    // Check if FR version already exists for this video
